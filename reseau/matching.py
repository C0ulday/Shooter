...
class Matching:
    ...
    def detect_colors(self, frame):
        ...
        margin = 8
        hsv_ranges = [
            ([max(self.hsv1[0] - margin, 0), 50, 50], [min(self.hsv1[0] + margin, 179), 255, 255]),
            ([max(self.hsv2[0] - margin, 0), 50, 50], [min(self.hsv2[0] + margin, 179), 255, 255])
        ]
    ...
    def matching_check(self):
        ...
        combined_mask = self.detect_colors()
        ...
        if contours:
            ...
            match_result = self.compare_contours(largest_contour, self.reference_contour, threshold=0.5)
            if match_result:
